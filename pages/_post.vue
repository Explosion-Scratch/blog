  ...
  <div class="app_container max-w-[600px] w-5/6">
    ...
    <div id="info" class="mb-4 w-full">
      ...
      <span class="date">
        ...
        <template v-if="page.createdAt !== page.updatedAt"
          >(Edited {{ date_format.format(new Date(page.updatedAt)) }})</template
        >
      </span>
    ...
    if (process.browser) {
      ...
      document.addEventListener("click", (e) => {
        if (["H1", "H2", "H3"].includes(e.target.tagName)) {
          if (
            !e.target.id ||
            !document.querySelector(".app_container").contains(e.target)
          )
            return;
