    ...
    <template v-if="title && link">
      ...
      <div class="right">
        <h3>
          <img
            class="inline !m-0"
            :src="`https://www.google.com/s2/favicons?domain=${host(link)}`"
          />
          {{ slice(title, 6) }}
        </h3>
        ...
        <br />
        <a :href="link" class="visit"> Visit </a>
      </div>
    </template>
    <a
      :href="url"
      class="text-lg loading cursor-pointer flex items-center w-full p-3 hover:text-green-200 bg-opacity-95 transition-colors"
      v-else
    >
      <svg
        class="mr-1 inline"
      ...
      >
        <path
          d="M208.61 118.1l-28.283 28.285a50.057 50.057 0 0 1-70.711 0a6 6 0 0 1 8.484-8.485a38.046 38.046 0 0 0 53.741 0l28.284-28.285a38 38 0 0 0-53.74-53.74l-19.8 19.799a6 6 0 0 1-8.484-8.485L137.9 47.391a50 50 0 1 1 70.71 70.709zm-79.195 62.226l-19.8 19.799a38 38 0 1 1-53.74-53.74L84.16 118.1a38.046 38.046 0 0 1 53.74-.001a6 6 0 0 0 8.485-8.485a50.055 50.055 0 0 0-70.71 0L47.39 137.9a50 50 0 0 0 70.71 70.71l19.799-19.8a6 6 0 1 0-8.484-8.484z"
          fill="currentColor"
        ></path>
      </svg>
      {{ url }}
...
export default {
  ...
  methods: {
    ...
    slice: (text, words) => {
      return text.split(" ").slice(0, words).length === words
    ...
    handleError(img_el) {
      if (img_el.src.startsWith("https://cors.explosionscratc.repl.co")) {
        ...
        console.log(img_el);
        return (img_el.src = `https://cors.explosionscratc.repl.co/${
          img_el.src.split("//")[1]
        }`);
      }
    },
    parseMeta(m) {
      return {
        ...
        description:
          m.description || m?.og?.description || m?.twitter?.description,
    ...
    async meta(url) {
      ...
      var html = (window.html = parser.parseFromString(
        await fetch(
      ...
      [...html.querySelectorAll("meta[property], meta[name]")]
        .filter((i) =>
          /^[^:]+:[^:]+/.test(
            i.getAttribute("property") || i.getAttribute("name")
          )
        )
        .map((i) => {
...
.right .visit:hover {
  box-shadow: 0 0 0 2px rgba($base-color, 0.1);
}
